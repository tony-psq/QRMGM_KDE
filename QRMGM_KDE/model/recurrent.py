...
class MGMCell(Layer):
    """Cell class for the MGM layer.
    Minimal Gated Memory Network
    https://doi.org/10.1016/j.enconman.2019.06.024
    ...
    def __init__(self, units,
                 ...
                 implementation=1,
                 **kwargs):
        super(MGMCell, self).__init__(**kwargs)
        self.units = units
    ...
    def build(self, input_shape):
        ...
        self.kernel_net = self.kernel[:, :self.units]

        self.recurrent_kernel_net = self.recurrent_kernel[:, :self.units]

        if self.use_bias:
            self.bias_net = self.bias[:self.units]
        else:
            self.bias_net = None
        self.built = True
    ...
    def call(self, inputs, states, training=None):
        ...
        if self.implementation == 1:
            if 0 < self.dropout < 1.:
                inputs_net = inputs * dp_mask[0]
            else:
                inputs_net = inputs
            x_net = K.dot(inputs_net, self.kernel_net)
            if self.use_bias:
                x_net = K.bias_add(x_net, self.bias_net)

            if 0 < self.recurrent_dropout < 1.:
                h_tm1_net = h_tm1 * rec_dp_mask[0]
            else:
                h_tm1_net = h_tm1
            f = self.recurrent_activation(x_net + K.dot(h_tm1_net,
                                                      self.recurrent_kernel_net))
            a = self.activation(x_net + K.dot(h_tm1_net,
                                                      self.recurrent_kernel_net))
        else:
            if 0. < self.dropout < 1.:
                inputs *= dp_mask[0]
    ...
    def get_config(self):
        ...
        base_config = super(MGMCell, self).get_config()
        return dict(list(base_config.items()) + list(config.items()))



class MGM(RNN):
    """Minimal Gated Memory neural network -zzd
    https://doi.org/10.1016/j.enconman.2019.06.024
    ...
    def call(self, inputs, mask=None, training=None, initial_state=None):
        ...
        return super(MGM, self).call(inputs,
                                      mask=mask,
                                      training=training,
    ...
    def get_config(self):
        config = {'units': self.units,
                  ...
                  'recurrent_dropout': self.recurrent_dropout,
                  'implementation': self.implementation}
        base_config = super(MGM, self).get_config()
